/**
 * problem: https://codeforces.com/contest/1632/problem/D
 * author: Nabajyoti
 */
#include <algorithm>
#include <bitset>
...
// ----- end of utilities ------ //
ll n;
bool check(ll ql, SGT& sgt, ll qr) {
    // cout<<"checking for ql and qr = {"<<ql<<", "<<qr<<"}\n";
    ll len = qr - ql + 1;
    ll g = sgt.range_query(ql, qr);
    // printf(". g = %lld and len = %lld\n", g, len);
    return g >= len;
}
...
void solve() {
    ...
    for (ll i = 0; i < n; i++) {
        ...
        // do binary search
        ll lo = lastP + 1, hi = i;
        ll ans_b = i;  // ans of binary search
        while (lo <= hi) {
            ll mid = (lo + hi) / 2;
            if (check(mid, sgt, i)) {
                ans_b = mid;
        ...
        if (sgt.range_query(ans_b, i) == (i - ans_b + 1)) {
            ans[i]++;
            lastP = i;
        }
    }
